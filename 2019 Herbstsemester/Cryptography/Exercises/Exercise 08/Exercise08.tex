...
\begin{document}
	...
	\begin{enumerate}[]
		...
		\begin{tabular}{lll}
			\begin{tabular}{|l|}
				\hline \cellcolor[gray]{0.8}$Enc_L(k,(m_L, m_R)):$ \\
		 		\ \ $r \leftarrow \{ 0,1 \} ^{\lambda}$ \\
		 		\ \ $z := F(k,m_L) \oplus r$ \\
		 		\ \ \textbf{return} $(r,z)$ \\ \hline
			\end{tabular}
			...
			&
			\begin{tabular}{|l|}
				\hline \cellcolor[gray]{0.8}$Enc_R(k,(m_L, m_R)):$ \\
		 		\ \ $r \leftarrow \{ 0,1 \} ^{\lambda}$ \\
		 		\ \ $z := F(k,m_R) \oplus r$ \\
		 		\ \ \textbf{return} $(r,z)$ \\ \hline
			\end{tabular}			
		...
		\begin{tabular}{ll}
			...
			&
			\begin{tabular}{p{7.5cm}}
				First we will pick a key $k$ and two messages $x$ and $y$ at random. Then the return values of $Enc(k,m)$ are fetched. In the end the XOR-step which the encoder has done, will be reversed by using the return values. The distinguisher will therefore output whether both $F(k,m)$ have returned the same value.
			\end{tabular}
		\end{tabular}
		\\ \\ \\
		It is obvious that if the $Enc_L$ is used the adversary will output 1 because in both cases $F(k,x)$ is returned. If the $Enc_R$ is used one will output $F(k,x)$ and the other one will output $F(k,y)$ which will not be the same and therefore the output value of A will be 0. So the adversary can distinguish between $Enc_L$ and $Enc_R$ and therfore this encoder is not \textit{CPA-secure}.
	\end{enumerate}
	...
	\begin{enumerate}[]
		...
		\begin{tabular}{lll}
			\begin{tabular}{|l|}
				\hline \cellcolor[gray]{0.8}$Enc_{CPA-real}(k,(m_L, m_R)):$ \\
		 		\ \ $r \leftarrow \{ 0,1 \} ^{\lambda}$ \\
		 		\ \ $s := r \oplus m$ \\
		 		\ \ $x := F(k,r)$ \\
		 		\ \ \textbf{return} $(s, x)$ \\ \hline
			...
			&
			\begin{tabular}{|l|}
				\hline \cellcolor[gray]{0.8}$Enc_{CPA-rand}(k, m):$ \\
		 		\ \ $x \leftarrow (\{ 0,1 \} ^{\lambda})^{2}$ \\
		 		\ \ \textbf{return} $x$ \\ \hline
			\end{tabular}			
		\end{tabular} 
		\\ \\ \\
		\begin{tabular}{llll}
			\begin{tabular}{|l|}
				\hline \cellcolor[gray]{0.8}$Enc_{CPA-real}(k,m):$ \\
		 		\ \ $r \leftarrow \{ 0,1 \} ^{\lambda}$ \\
		 		\ \ $s := r \oplus m$ \\
		 		\ \ $x := F(k,r)$ \\
		 		\ \ \textbf{return} $(s, x)$ \\ \hline
			...
			&
			\begin{tabular}{|l|}
				\hline \cellcolor[gray]{0.8}$Enc_{CPA-hybrid1}(k,m):$ \\
		 		\ \ $r \leftarrow \{ 0,1 \} ^{\lambda}$ \\
		 		\ \ $s := r \oplus m$ \\
			...
			&	
			\begin{tabular}{p{5cm}}
				Because we know that F is a secure PRP/PRF we can inline the subroutine of a random PRP/PRF.
			\end{tabular}	
		\end{tabular}
		\\ \\ \\
		\begin{tabular}{llll}
			...
			&
			\begin{tabular}{|l|}
				\hline \cellcolor[gray]{0.8}$Enc_{CPA-hybrid2}(k,m):$ \\
		...
		\begin{tabular}{llll}
			...
			&	
			\begin{tabular}{p{5cm}}
				Because s and x are making up a pair which is in $C$ we can inline such a subroutine to end up with $Enc_{CPA-rand}(k, m):$.
			\end{tabular}	
		\end{tabular}
	\end{enumerate}
