...
public:
    ...
    static Vec solve(ProblemBase* _problem, const double _eps = 1e-4, const int _max_iters = 1000000)
    {
        ...
        do {
            ...
            if (lambda2/2.0 <= e2) break;

            // step size
            double t(1.0);
            t = LineSearch::backtracking_line_search(_problem, x, g, delta_x, t);
                                            ...
                                            const double _eps = 1e-4, const int _max_iters = 1000000 )
    {
        ...
        do {
            ...
            int cnt = 0;
            double tau = 2.0;
            bool is_not_psd = !H.ldlt().isPositive();
            while (is_not_psd && cnt < _max_iters) {
                if (cnt==0) {
                    tau = 1e-3 * std::abs(H.trace()) / double(n);
                }
                H += tau*I;
                is_not_psd = !H.ldlt().isPositive();
                cnt++;
                tau *= tau_factor;
            }
            delta_x = H.ldlt().solve(-g);
