...
namespace AOPT {
    double MassSpringSystem::initial_system_energy() const{
        if(msp_.get() == nullptr)
            return -1;
    ...
    void MassSpringSystem::set_spring_graph_vertices(const Vec& _points) {
    ...
    void MassSpringSystem::save_spring_system(const char *_filename) const {
        sg_.save_to_files(_filename);
    }


    std::shared_ptr<AOPT::MassSpringProblem2D> MassSpringSystem::get_problem() const {
        return msp_;
    }

    AOPT::SpringGraph& MassSpringSystem::get_spring_graph() {
        return sg_;
    }
    ...
    void MassSpringSystem::setup_problem(const int _spring_element_type) {
        ...
        msp_.get()->add_fixing_node_element(get_grid_index(n_grid_x_, n_grid_y_), 100000, 10.*n_grid_x_/3, n_grid_y_);
        msp_.get()->add_fixing_node_element(get_grid_index(n_grid_x_, 0), 100000, 5*n_grid_x_, 0);
        msp_.get()->add_fixing_node_element(get_grid_index(0, n_grid_y_), 100000, 5.*n_grid_x_/3, n_grid_y_);

//        msp_.get()->add_fixing_node_element(get_grid_index(n_grid_x_/2, n_grid_y_/2), 100000, 2*n_grid_x_, 0.8*n_grid_y_);

//        for (int i = 0; i <= n_grid_x_; ++i) {
//            msp_.get()->add_fixing_node_element(get_grid_index(i, 0), 100000, i, 0);
//            msp_.get()->add_fixing_node_element(get_grid_index(i, n_grid_y_), 100000, i, n_grid_y_);
//        }
//        //------------------------------------------------------//
    }
...
//                sg_.add_vertex(Point(i, j));
    ...
    MassSpringSystem::Vec
    MassSpringSystem::get_spring_graph_points() const {
