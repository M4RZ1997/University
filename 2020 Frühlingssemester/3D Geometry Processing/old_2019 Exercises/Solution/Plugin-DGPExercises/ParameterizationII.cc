...
//    // finding the best fitting plane of the boundary
//    std::pair<Vec3d, Vec3d> plane = best_plane_from_points(boundary_vertices);
//    Vec3d pnrm = plane.second;
//    Vec3d pct = plane.first;
//    Vec3d y_axis(0, 0, 0), x_axis(0, 0, 0);
//    x_axis = cross(Vec3d(0, 1, 0), pnrm);
//    if(norm(y_axis) < 1e-5) x_axis = cross(Vec3d(1, 0, 0), pnrm); x_axis.normalize();
//    y_axis = cross(pnrm, x_axis);y_axis.normalize();
//
//    // give each vertex a uv coordinate
...
//        {
//            v_texture[v] = Vec2d(dot(Vec3d(p) - pct, x_axis), dot(Vec3d(p) - pct, y_axis));
//            v_texture[v].normalize();
//            v_texture[v] = v_texture[v] / 2 + Vec2d(0.5, 0.5);
//        }
//        else
//        {
//            v_texture[v] = Vec2d(0.5, 0.5);
//        }
//    }
...
void ParameterizationII::explicitly_smooth_texture(const Point& _origin, const int _num_iters)
{
    ...
    for (auto vh : mesh_.vertices()) {
        auto tex_coord = mesh_.texcoord2D(vh);
//        std::cerr<<" "<<tex_coord[0]<<" "<<tex_coord[1];
        tex_mesh_.set_point(vh, Point(tex_coord[0], tex_coord[1], 0) + _origin);
    }
}
