...
void DGPExercisePlugin::initializePlugin()
{
    ...
    connect(tool_->select_vhs_pb, SIGNAL(clicked()), this, SLOT(slot_select_vertices()));
    connect(tool_->show_displacement_pb, SIGNAL(clicked()), this, SLOT(slot_visualize_displacement_vector()));
    connect(tool_->deform_pb, SIGNAL(clicked()), this, SLOT(slot_deform_mesh()));
...
//            pmr.solve_harmonic_function(constraint_vhs1, "harmonic functions1");
...
//            emit addEmptyObject(DATA_TETRAHEDRAL_MESH, iso_line_obj1);
//            auto mesh_obj1 = PluginFunctions::tetrahedralMeshObject(iso_line_obj1);
//            mesh_obj1->materialNode()->set_line_width(5);
//            pmr.compute_isolines(*mesh_obj1->mesh(), constraint_vhs1, "harmonic functions1", tool_->num_intervals_sb->value());
...
//======================================================================================================================
void DGPExercisePlugin::slot_select_vertices() {
    for (PluginFunctions::ObjectIterator o_it(PluginFunctions::TARGET_OBJECTS, DATA_TRIANGLE_MESH);
         o_it != PluginFunctions::objectsEnd(); ++o_it) {
        auto tri_obj = PluginFunctions::triMeshObject(*o_it);
        auto trimesh = tri_obj->mesh();
        ...
        if (trimesh) {
            ...
            if(tool_->vertex_type_cb->currentIndex() == 0) {
                //reset
                fixed_vhs_.clear();
                for(auto vh : trimesh->vertices())
                    if(trimesh->color(vh) != ACG::Vec4f(0,0,1,1))
                        trimesh->set_color(vh, ACG::Vec4f(1,1,1,0));

                for(auto vh : trimesh->vertices()) {
                    if(trimesh->status(vh).selected()) {
                        fixed_vhs_.push_back(vh.idx());
                        trimesh->status(vh).set_selected(false);
                        trimesh->set_color(vh, ACG::Vec4f(1,1,0,1));
                    }
                }
            } else {
                //reset
                displaced_vhs_.clear();
                for(auto vh : trimesh->vertices())
                    if(trimesh->color(vh) != ACG::Vec4f(1,1,0,1))
                        trimesh->set_color(vh, ACG::Vec4f(1,1,1,0));

                for(auto vh : trimesh->vertices()) {
                    if(trimesh->status(vh).selected()) {
                        displaced_vhs_.push_back(vh.idx());
                        trimesh->status(vh).set_selected(false);
                        trimesh->set_color(vh, ACG::Vec4f(0,0,1,1));
                    }
                }
...
void DGPExercisePlugin::slot_visualize_displacement_vector() {
    for (PluginFunctions::ObjectIterator o_it(PluginFunctions::TARGET_OBJECTS, DATA_TRIANGLE_MESH);
         o_it != PluginFunctions::objectsEnd(); ++o_it) {
        auto tri_obj = PluginFunctions::triMeshObject(*o_it);
        auto trimesh = tri_obj->mesh();

        if (trimesh && !displaced_vhs_.empty()) {
            std::stringstream ss(tool_->displacemesh_le->text().toStdString().c_str());
            ACG::Vec3d displacement(0.);
            for(int i=0; i<3; ++i)
                ss >> displacement[i];
            //end points of line
            ACG::Vec3d p0(0.), p1(0.);
            for(auto vh : displaced_vhs_)
                p0 += trimesh->point(OpenMesh::VertexHandle(vh));
            p0 /= displaced_vhs_.size();
            p1 = p0 + displacement;

            ACG::SceneGraph::LineNode* lineNode;
            //create line node
            if (!tri_obj->getAdditionalNode(lineNode, name(),"Displacement Vector"))
            {
                lineNode = new ACG::SceneGraph::LineNode(ACG::SceneGraph::LineNode::LineSegmentsMode,
                        tri_obj->manipulatorNode(),"Displacement Vector");
                tri_obj->addAdditionalNode(lineNode, name(), "Displacement Vector");
...
void DGPExercisePlugin::slot_deform_mesh() {
    for (PluginFunctions::ObjectIterator o_it(PluginFunctions::TARGET_OBJECTS, DATA_TRIANGLE_MESH);
         o_it != PluginFunctions::objectsEnd(); ++o_it) {
        auto tri_obj = PluginFunctions::triMeshObject(*o_it);
        auto trimesh = tri_obj->mesh();
        ...
        if (trimesh) {
            ...
            Deformation dfm(*trimesh);
            dfm.deform(fixed_vhs_, displaced_vhs_, displacement);
